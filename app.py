...
try:
    ...
    st.error("Por favor, sube los archivos 'rfm_segmentation.csv' y 'df_merged_with_segments.csv'.")
    st.stop()
...
def get_basket_analysis(dataframe):
    ...
    df_uk_sample = df_uk.head(1000)  # limitar filas para evitar sobrecarga en Streamlit Cloud
    basket = (df_uk_sample.groupby(['Invoice', 'Description'])['Quantity']
...
# Convertir frozenset a strings para evitar errores en Streamlit
rules_display = rules.copy()
rules_display['antecedents'] = rules_display['antecedents'].apply(lambda x: ', '.join(list(x)))
rules_display['consequents'] = rules_display['consequents'].apply(lambda x: ', '.join(list(x)))

st.markdown("### Reglas de Asociación (Top 10)")
st.dataframe(rules_display[['antecedents', 'consequents', 'support', 'confidence', 'lift']].head(10), use_container_width=True)
st.markdown("""
